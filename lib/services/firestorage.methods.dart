...
class FireStorageMethods {
    ...
    if (filepath.isEmpty) {
      ...
      try {
        ...
        if (file.lengthSync() > fileLimitSize15MB) {
          showSnackbar(context, 'Your image must not exceed 15MB', null);
  ...
  static Future<List> uploadimageToEventCover(context, String filepath) async {
    ...
    //
      ...
      try {
        ...
        if (file.lengthSync() > fileLimitSize15MB) {
          Navigator.of(context).pop();
          showSnackbar(context, 'Your cover image must not exceed 15MB', null);
          log('FILE SIZE ${file.lengthSync() / 1000000}');
          return [false, ''];
        }
        //
  ...
  //
  {
    ...
    if (file.existsSync() == false) {
      // ignore: use_build_context_synchronously
      showSnackbar(context, 'This file no longer exists!', null);
    ...
    if (type == 'image' || type == 'video') {
      if (thumbailFile.existsSync() == false) {
        // ignore: use_build_context_synchronously
        showSnackbar(context, 'An error occurred with the file!', null);
    ...
    if (file.lengthSync() > fileLimitSize15MB) {
      // ignore: use_build_context_synchronously
      showSnackbar(context, 'Your file must not exceed 15MB', null);
      log('FILE SIZE ${file.lengthSync() / 1000000}');
      progressStreamController.add(-1);
      yield* progressStreamController.stream;
      return;
