  ...
  initState() {
    ...
    streamCurrentUserSubscription = streamCurrentUser.asBroadcastStream().listen((event) {
      ...
      streamEventsAndRemindersSubscription = streamEventsAndReminders.asBroadcastStream().listen((event) {
        ...
        initAppointments(eventsList: eventsList, remindersList: remindersList);
      });
    });
  }
        ...
        children: [
          Column(
            children: [
              ...
              //             backgroundColor: kSecondColor.withOpacity(0.3),
              //             color: kSecondColor,
              //           )
              //         : Container();
              //   },
              ...
              Expanded(
                child: SfCalendar(
                  ...
                  todayTextStyle: TextStyle(locale: const Locale('fr', 'FR'), color: Colors.white, fontSize: 14.sp),
                  ...
                  onTap: (calendarTapDetails) {
                    log('sfC: ${calendarTapDetails.date}');
                  },
                  headerHeight: 0,
            ...
            builder: (context, value, child) {
              return isUpdating.value
                  ? Container(
                      ...
                      alignment: Alignment.center,
                      child: Center(
                        child: CupertinoActivityIndicator(radius: 14.sp, color: Colors.white70),
                      ),
                    )
                  : Container();
            },
...
class buildEventContainer extends StatefulWidget {
  ...
  const buildEventContainer({Key? key, required this.appointment, this.currentUser}) : super(key: key);

  @override
  State<buildEventContainer> createState() => _buildEventContainerState();
}

class _buildEventContainerState extends State<buildEventContainer> {
  String itemType = '';
  String contentId = '';
  String userPosterId = '';
  String eventType = '';
                                                ...
                                                Wrap(
                                                  children: [
                                                    Text(
                                                      widget.appointment.isAllDay
                                                          ? 'Toute la journée'
                                                          : '${widget.appointment.startTime.hour.toString().padLeft(2, "0")}:${widget.appointment.startTime.minute.toString().padLeft(2, "0")} à ${widget.appointment.endTime.hour.toString().padLeft(2, "0")}:${widget.appointment.endTime.minute.toString().padLeft(2, "0")}',
                                                      overflow: TextOverflow.ellipsis,
...
class buildCalendarScheduleHeader extends StatelessWidget {
  ...
  final usermodel.User? currentUser;
  const buildCalendarScheduleHeader({Key? key, required this.details, this.currentUser}) : super(key: key);
    ...
    return Container(
        decoration: BoxDecoration(
          image: DecorationImage(
              ...
              fit: BoxFit.cover,
              colorFilter: ColorFilter.mode(Colors.grey.shade200.withOpacity(.5), BlendMode.darken)),
        ...
        child: Container(
          ...
          child: Padding(
            ...
            child: Text(
              ReCase(DateFormat('MMMM yyyy', 'fr_Fr').format(details.date)).sentenceCase,
